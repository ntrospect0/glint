...
impl EmailWidget {
    ...
    /// Toggle expanded state on the selected message. When expanding, also
    /// mark the message as seen-via-glint and persist the seen-store.
...
fn build_provider(name: &str) -> (EmailProviderHandle, String, bool, Option<String>) {
    match name.to_ascii_lowercase().as_str() {
        ...
        other => (
            ...
            other.to_string(),
            false,
            Some(format!("unknown provider {other:?} (expected outlook or gmail)")),
        ),
    }
}
...
impl Widget for EmailWidget {
    ...
    fn render(&self, frame: &mut Frame, area: Rect, focused: bool) {
        ...
        // When expanded: subject + up to MAX_SUMMARY_LINES wrapped body lines underneath.
        const ROWS_PER_ITEM: usize = 1;
        let list_height = list_area.height;
        let items_visible = (list_height as usize / ROWS_PER_ITEM).max(1);
        // Keep the selected message in view, but never snap it to the
        // top — expanding (e) should grow the row in place so the user
        // doesn't lose context above. If the expanded body overflows
        // the pane bottom, it gets clipped (Up/Down scrolls to see).
        if selected < scroll {
            scroll = selected;
        }
...
fn expanded_body_lines(
    ...
    if llm_enabled {
        if let Some(s) = summary_state {
            match s {
                SummaryState::Ready(text) => return wrap_text(&text, max_width, MAX_SUMMARY_LINES),
