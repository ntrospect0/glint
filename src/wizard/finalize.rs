...
fn render_llm_toml(
    ...
    let prev_provider = prev
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let same_provider = prev_provider == provider.name;
    let enabled = prev
        .as_ref()
        .and_then(|d| d.get("enabled"))
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    let model = same_provider
        .then(|| {
            prev.as_ref()
                .and_then(|d| d.get("provider"))
    ...
    let max_tokens = same_provider
        ...
        .flatten()
        .unwrap_or(provider.default_max_tokens);
    let (rpm, cache_capacity) = {
        let limits = prev.as_ref().and_then(|d| d.get("limits"));
        (
            limits
