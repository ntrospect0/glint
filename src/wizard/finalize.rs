...
//! - `~/.config/glint/credentials/anthropic_key.toml` (if a key was set)
//!
//! Widgets whose wizard descriptors have no fields (Phase A's
//! `defer_to_toml` stubs) keep their existing TOML untouched when the file
//! exists; otherwise they're seeded from the `DEFAULT_*_TOML` constants in
//! `config::mod`. Widgets with fields render their TOML from the wizard
//! values.
...
pub fn write_all(state: &WizardState) -> Result<()> {
    ...
    if let Some(key) = api_key_from_state(state) {
...
fn api_key_from_state(state: &WizardState) -> Option<String> {
    match state.global_get("llm_api_key") {
        Some(WizardValue::Text(s)) if !s.trim().is_empty() => Some(s.trim().to_string()),
        _ => None,
    }
}

fn write_anthropic_key(key: &str) -> Result<()> {
    let dir = auth::credentials_dir()?;
    let path = dir.join("anthropic_key.toml");
    let body = format!(
        "# Anthropic API key. Generated by `glint --setup`.\n\
         api_key = {}\n",
        toml_string(key)
    );
    atomic_write(&path, &body)
...
fn rewrite_widget_line(block: &str, new_widget: &str) -> String {
    let mut out = String::with_capacity(block.len() + new_widget.len());
    ...
    for line in block.split_inclusive('\n') {
        if !replaced {
            ...
            if trimmed.starts_with("widget") && line_assigns_key("widget", trimmed) {
                ...
                out.push_str("widget = ");
                out.push_str(&toml_string(new_widget));
                out.push('\n');
                replaced = true;
                continue;
            }
